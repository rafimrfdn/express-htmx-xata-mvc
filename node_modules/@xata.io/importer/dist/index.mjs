import { XataFile, branchTransaction, putFile, XataPlugin } from '@xata.io/client';
import AnyDateParser from 'any-date-parser';
import CSV from 'papaparse';
import pick from 'lodash.pick';
import JSON$1 from 'json5';
...
class XataImportPlugin extends XataPlugin {
  build(pluginOptions) {
    return {
      parseCsvStream,
...
function randomData(column) {
  switch (column.type) {
    case "text":
      return fakerEN.lorem.paragraphs(rand(2, 3));
    case "email":
      return fakerEN.internet.email({ provider: "acme.pets" });
    case "int":
      return rand(1, 100);
    case "float":
    ...
    case "bool":
      return rand(0, 1) === 1;
    case "multiple":
      return fakerEN.word.words(rand(1, 3)).split(" ");
    case "string":
      return randomString(column.name);
    case "datetime":
      return fakerEN.date.recent({ days: rand(1, 10) });
    default:
      return void 0;
  }
}
...
export { DEFAULT_CSV_DELIMITERS_TO_GUESS, XataImportPlugin, generateRandomData, guessColumnTypes, importColumnTypes, isValidEmail, parseCsvStream, parseCsvStreamBatches };
