...
const pgrollMigration3 = {
    ...
    migration: '{"name": "mig_xbcdcdrj7c92neg7lefx", "operations": [{"sql": { "up": "ALTER ALTER TABLE internal ADD test varchar(255)" }}]}',
...
const pgrollFetchSingle = (url, request, type = 'pgroll') => {
...
const pgrollFetchEmpty = (url, request) => {
    if (url === `${baseUrl}` && request.method === 'GET') {
        return {
            ok: true,
            json: async () => ({
    ...
    describe('for Xata 2.0 branches', () => {
        ...
        describe('inferred migration format', () => {
            test('combines new local migrations with existing remote migrations', async () => {
                ...
                vi.spyOn(fs, 'readFile').mockImplementationOnce(async () => pgrollMigration1.name + '\n' + pgrollMigration3.name);
                vi.spyOn(fs, 'readFile').mockImplementationOnce(async () => JSON.stringify(hydrateMigrationObject(pgrollMigration3)));
                vi.spyOn(fs, 'readFile').mockImplementationOnce(async () => JSON.stringify(hydrateMigrationObject(pgrollMigration3)));
                vi.spyOn(fs, 'readFile').mockImplementationOnce(async () => pgrollMigration1.name + '\n' + pgrollMigration3.name);
                vi.spyOn(fs, 'readFile').mockImplementationOnce(async () => JSON.stringify(hydrateMigrationObject(pgrollMigration3)));
                vi.spyOn(fs, 'readFile').mockImplementationOnce(async () => JSON.stringify(hydrateMigrationObject(pgrollMigration3)));
            ...
            test('does not push migrations remote if they already exist', async () => {
                ...
                const command = new Push(['main'], config);
                const log = vi.spyOn(command, 'log');
                vi.spyOn(fs, 'readdir').mockImplementation(async () => [pgrollMigration3.name]);
                vi.spyOn(fs, 'readFile').mockImplementationOnce(async () => JSON.stringify(hydrateMigrationObject(pgrollMigration3)));
