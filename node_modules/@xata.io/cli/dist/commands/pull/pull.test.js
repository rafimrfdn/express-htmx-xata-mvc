...
const pgrollMigration2 = {
    done: true,
    migration: '{"name": "mig_abcdcdrj7c92neg7lefg", "operations": [{"sql": { "up": "ALTER ALTER TABLE internal ADD test varchar(255)" }}]}',
    migrationType: 'inferred',
    name: 'mig_abcdcdrj7c92neg7lefg',
    parent: 'mig_abcdcdrj7c92neg7lerr',
    startedAt: '2024-01-18T14:31:20.795975Z'
};
const pgrollMigration3 = {
    done: true,
    migration: '{"name": "mig_abcdcdrj7c92neg7lefg", "operations": [{"drop_column": {"down": "", "table": "tester", "column": "Firstname"}}]}',
    migrationType: 'pgroll',
    name: 'mig_abcdcdrj7c92neg7lefg',
    parent: 'mig_abcdcdrj7c92neg7lerr',
    startedAt: '2024-01-18T14:31:20.795975Z'
};
const pgrollMigration4 = {
    done: true,
    migration: '{"name": "mig_xyzdcdrj7c92neg7lxyz", "operations": [{"drop_column": {"down": "", "table": "tester", "column": "Firstname"}}]}',
    migrationType: 'pgroll',
...
const fetchMultiple = (url, request) => {
...
const pgrollFetchSingle = (url, request) => {
...
const pgrollFetchMultiple = (url, request) => {
    if (url === `${baseUrl}` && request.method === 'GET') {
        return {
            ok: true,
            json: async () => ({
...
describe('pull', () => {
    describe('for Xata 1.0 branches', () => {
        ...
        test('combines new remote migrations with existing local migrations', async () => {
            ...
            const log = vi.spyOn(command, 'log');
            vi.spyOn(fs, 'readdir').mockImplementation(async () => [regularMigration1.id]);
    ...
    describe('for Xata 2.0 branches', () => {
        ...
        test('combines new remote migrations with existing local migrations when they are both in the correct format', async () => {
            ...
            const command = new Pull(['main'], config);
            const log = vi.spyOn(command, 'log');
            vi.spyOn(fs, 'readdir').mockImplementation(async () => [pgrollMigration1.name]);
            vi.spyOn(fs, 'readFile').mockImplementationOnce(async () => JSON.stringify(hydrateMigrationObject(pgrollMigration1)));
        ...
        test('overwrites all old migrations if they are in the wrong format', async () => {
            ...
            vi.spyOn(fs, 'readdir').mockImplementationOnce(async () => [regularMigration1.id]);
        ...
        test('allMigrationsPgRollFormat helper', async () => {
            vi.spyOn(fs, 'readdir').mockImplementationOnce(async () => [pgrollMigration1.name]);
            vi.spyOn(fs, 'readFile').mockImplementationOnce(async () => pgrollMigration1.name);
            vi.spyOn(fs, 'readFile').mockImplementationOnce(async () => JSON.stringify(hydrateMigrationObject(pgrollMigration1)));
        ...
        describe('inferred migration format', () => {
            test('allMigrationsPgRollFormat helper', async () => {
                vi.spyOn(fs, 'readdir').mockImplementationOnce(async () => [pgrollMigration2.name]);
                vi.spyOn(fs, 'readFile').mockImplementationOnce(async () => pgrollMigration2.name);
                vi.spyOn(fs, 'readFile').mockImplementationOnce(async () => JSON.stringify(hydrateMigrationObject(pgrollMigration2)));
                expect(await allMigrationsPgRollFormat()).toBe(true);
            });
        });
    });
