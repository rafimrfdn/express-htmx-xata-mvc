...
class RandomData extends BaseCommand {
    async run() {
        ...
        if (schemaTables.length === 0) {
            this.warn(`Your database has no tables. To create one, use ${chalk.bold('xata schema edit')}. Once your database has at least one table, running this command again will generate random data for you.`);
            this.log();
        }
        ...
        for (const table of tables) {
            ...
            this.info(`Inserted ${chalk.bold(totalRecords)} random ${pluralize('record', totalRecords)} in the ${chalk.bold(table.name)} table`);
        }
        this.success(`Inserted ${chalk.bold(tables.length * totalRecords)} random records across ${chalk.bold(tables.length)} ${pluralize('table', tables.length)}`);
    }
}
_a = RandomData;
RandomData.description = 'Insert random data in the database';
RandomData.examples = [];
RandomData.flags = {
    ..._a.databaseURLFlag,
