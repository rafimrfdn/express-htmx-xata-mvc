...
const fetchImplementation = (url, request) => {
...
const isGitIgnoredMock = isIgnored;
const spawnMock = spawn;
const runInitTest = async (files, args = [], prompts = { workspace: 'test-1234', database: 'db1' }, setupCommand = () => null) => {
...
describe('xata init', () => {
    ...
    test('generates typescript types', async () => {
        ...
        spawnMock.mockReturnValue({ on: (string, func) => func(0) });
    ...
    test('warns user if no package.json', async () => {
        ...
        expect(warn.mock.calls.flat()).toEqual(expect.arrayContaining([
            expect.stringContaining('No package.json found. Please run one of: pnpm init, yarn init, npm init, bun init. Then rerun')
        ]));
        expect(outputFiles).toMatchInlineSnapshot(`
      {
        ".env": "# [Xata] Configuration used by the CLI and the SDK
    ...
    test('uses pnpm automatically if lockfile exists ', async () => {
        ...
        vi.spyOn(which, 'sync').mockImplementation(() => '/usr/bin/pnpm');
        spawnMock.mockImplementation(() => ({ on: (string, func) => func(0) }));
        const { outputFiles, promptsMock } = await runInitTest(files, [], {
            workspace: 'test-1234',
            database: 'db1',
            codegen: 'ts',
