...
const isPackageManagerInstalled = (packageManager) => which.sync(packageManager.command, { nothrow: true });
const installXataClientCommand = (packageManager) => {
    const { command, args } = packageManager;
    return `${command} ${args} @xata.io/client`;
};
const readEnvFile = async (envFile) => {
    let content = '';
    try {
...
class Init extends BaseCommand {
    async run() {
        ...
        if (this.projectConfigLocation) {
            if (!flags.force) {
                this.error(`Project already configured at ${this.projectConfigLocation}. Use ${chalk.bold('--force')} to overwrite it`);
            }
            else {
        ...
        const branch = detectedBranch === DEFAULT_BRANCH
        ...
        this.log();
        if ((_c = (_b = this.projectConfig) === null || _b === void 0 ? void 0 : _b.codegen) === null || _c === void 0 ? void 0 : _c.output) {
            const { schema: currentSchema } = await (await this.getXataClient()).api.branches.getBranchDetails({ workspace, database, region, branch });
            const hasTables = (currentSchema === null || currentSchema === void 0 ? void 0 : currentSchema.tables) && (currentSchema === null || currentSchema === void 0 ? void 0 : currentSchema.tables.length) > 0;
            const hasColumns = currentSchema === null || currentSchema === void 0 ? void 0 : currentSchema.tables.some((t) => t.columns.length > 0);
            const isSchemaSetup = hasTables && hasColumns;
            if (shouldInstallPackage && !canInstallPackage) {
                this.warn(`No package.json found. Please run one of: pnpm init, yarn init, npm init, bun init. Then rerun ${chalk.bold('xata init --force')}`);
            }
            else if (!isSchemaSetup) {
                this.info(`Setup ${hasTables ? '' : 'tables and '}columns at https://app.xata.io/workspaces/${workspace}/dbs/${database}:${region}`);
                this.log();
                ...
                this.log(``);
                this.log(`import { getXataClient } from '${(_e = (_d = this.projectConfig) === null || _d === void 0 ? void 0 : _d.codegen) === null || _e === void 0 ? void 0 : _e.output}'`);
                this.log(``);
                this.log(`// server side query`);
                this.log(`await getXataClient().db${getDbTableExpression(currentSchema === null || currentSchema === void 0 ? void 0 : currentSchema.tables[0].name)}.getPaginated()`);
            }
        }
        else {
    ...
    async configureCodegen() {
        ...
        if (!output && !sdk) {
            ...
            if (['ts', 'esm', 'cjs', 'deno'].includes(codegen)) {
                ...
                if (!file)
                    return this.error('You must provide an output file');
    ...
    async getPackageManager() {
        ...
        if (!packageManager) {
            const { packageManagerName } = await this.prompt({
                ...
                choices: compact(Object.values(packageManagers).map((pm) => isPackageManagerInstalled(pm) ? { title: pm.command, value: pm.command } : null))
            });
            return packageManagers[packageManagerName];
        }
        if (packageManager && !isPackageManagerInstalled(packageManager)) {
            this.warn(`Looks like ${packageManager.command} is not installed or is not in the PATH. Please run ${chalk.bold(installXataClientCommand(packageManager))}`);
            return null;
        }
    ...
    async installPackage(packageManager, pkg) {
        const { command, args } = packageManager;
    ...
    async writeEnvFile(workspace, region, database, branch) {
        ...
        if (containsXataApiKey) {
            ...
            if (profile.host !== 'production')
                content += `XATA_API_PROVIDER=${buildProviderString(profile.host)}\n`;
    ...
    async waitUntilAPIKeyIsValid(workspace, region, database) {
...
_a = Init;
Init.description = 'Configure your working directory to work with a Xata database';
Init.examples = [
