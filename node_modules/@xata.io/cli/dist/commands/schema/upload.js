...
class UploadSchema extends BaseCommand {
    async run() {
        const { args, flags } = await this.parseCommand();
        const { workspace, region, database, branch } = await this.getParsedDatabaseURLWithBranch(flags.db, flags.branch, true);
        const xata = await this.getXataClient();
        if (flags['create-only']) {
            const { schema } = await getBranchDetailsWithPgRoll(xata, { workspace, region, database, branch });
            if (schema.tables.length > 0) {
                this.info('Schema already exists. `xata schema upload --init` will only initialize the schema if it does not already exist.');
                return;
            }
        }
...
_a = UploadSchema;
UploadSchema.description = 'Apply a schema to the current database from file';
UploadSchema.examples = [];
UploadSchema.flags = {
    ..._a.databaseURLFlag,
    ..._a.yesFlag,
