...
class EditSchema extends BaseCommand {
    ...
    async run() {
        ...
        if (flags.source) {
            this.warn(`This way of editing the schema doesn't detect renames of tables or columns. They are interpreted as deleting/adding tables and columns.
Beware that this can lead to ${chalk.bold('data loss')}. Other ways of editing the schema that do not have this limitation are:
...
* edit the schema in the Web UI. Use ${chalk.bold('xata browse')} to open the Web UI in your browser.`);
    ...
    async showSourceEditing(branchDetails) {
        const env = process.env.EDITOR || process.env.VISUAL;
        if (!env) {
            this.error(`Could not find an editor. Please set the environment variable ${chalk.bold('EDITOR')} or ${chalk.bold('VISUAL')}`);
        }
    ...
    async showInteractiveEditing(branchDetails) {
    ...
    async showSchema() {
        ...
        for (const table of this.tables) {
            const columnChoices = table.columns.map((column, i) => {
                if (this.selectItem === column)
                    index = flatChoices.length + i + 1;
                return {
            ...
            if (this.selectItem === table)
                index = flatChoices.length;
        ...
        select.on('keypress', async (char, key) => {
            ...
            try {
                if (key.name === 'backspace' || key.name === 'delete') {
                    if (!flatChoice)
                        return; // add table is not here for example
    ...
    getMessageForTable(table) {
        if (table.deleted)
            return `• ${chalk.red.strikethrough(table.name)}`;
        if (table.added)
            return `• ${chalk.green(table.name)}`;
        if (table.initialName)
            return `• ${chalk.bold(table.name)} ${chalk.yellow.strikethrough(table.initialName)}`;
        return `• ${chalk.bold(table.name)}`;
    }
    getMessageForColumn(table, column) {
        const linkedTable = this.tables.find((t) => { var _b; return (t.initialName || t.name) === ((_b = column.link) === null || _b === void 0 ? void 0 : _b.table); });
        function getType() {
            if (!linkedTable)
                return chalk.gray.italic(column.type);
        ...
        const metadata = [
            ...
            .filter(Boolean)
            .join(' ');
        if (table.deleted || column.deleted || (linkedTable === null || linkedTable === void 0 ? void 0 : linkedTable.deleted))
            return `- ${chalk.red.strikethrough(column.name)} (${metadata})`;
        if (table.added || column.added)
            return `- ${chalk.green(column.name)} (${metadata})`;
    ...
    getOverview() {
        ...
        for (const table of this.tables) {
            if (table.added)
                ...
                info.tables.modified++;
            for (const column of table.columns) {
                const linkedTable = this.tables.find((t) => { var _b; return (t.initialName || t.name) === ((_b = column.link) === null || _b === void 0 ? void 0 : _b.table); });
                if (table.added || column.added)
                    info.columns.added++;
                else if (table.deleted || column.deleted || (linkedTable === null || linkedTable === void 0 ? void 0 : linkedTable.deleted))
                    info.columns.deleted++;
                else if (column.initialName)
                    info.columns.modified++;
            }
        }
    ...
    async showColumnEdit(column, table) {
        var _b, _c, _d;
        this.clear();
        const isColumnAdded = !column || (column === null || column === void 0 ? void 0 : column.added);
        const template = `
           name: \${name}
           type: \${type}
           link: \${link}
vectorDimension: \${vectorDimension}
   ...
   defaultValue: \${defaultValue}`;
        const initial = {
            ...
            link: isColumnAdded ? '' : (_b = column === null || column === void 0 ? void 0 : column.link) === null || _b === void 0 ? void 0 : _b.table,
            vectorDimension: ((_c = column === null || column === void 0 ? void 0 : column.vector) === null || _c === void 0 ? void 0 : _c.dimension) ? `${(_d = column === null || column === void 0 ? void 0 : column.vector) === null || _d === void 0 ? void 0 : _d.dimension}` : undefined,
            notNull: (column === null || column === void 0 ? void 0 : column.notNull) ? 'true' : 'false',
            defaultValue: (column === null || column === void 0 ? void 0 : column.defaultValue) || '',
            unique: (column === null || column === void 0 ? void 0 : column.unique) ? 'true' : 'false',
            description: isColumnAdded ? '' : column === null || column === void 0 ? void 0 : column.description
        };
        const snippet = new Snippet({
            message: (column === null || column === void 0 ? void 0 : column.name) || 'a new column',
            initial,
            fields: [
                {
                    name: 'name',
                ...
                },
                {
                    ...
                    validate(rawDefaultValue, state, item, index) {
                        if (!isColumnAdded &&
                            state.values.type &&
  ...
  Description: \${description}`
    ...
    async deleteColumn(column, table) {
        if (column.added) {
    ...
    async migrate() {
        ...
        if (!this.branchDetails)
            this.error('Branch details are not available');
        ...
        for (const table of this.tables) {
            ...
            for (const column of table.columns) {
                const linkedTable = this.tables.find((t) => { var _b; return (t.initialName || t.name) === ((_b = column.link) === null || _b === void 0 ? void 0 : _b.table); });
                if (column.deleted || (linkedTable === null || linkedTable === void 0 ? void 0 : linkedTable.deleted)) {
...
_a = EditSchema;
EditSchema.description = 'Edit the schema of the current database';
EditSchema.examples = [];
EditSchema.flags = {
    ..._a.databaseURLFlag,
...
function validateOptionalBoolean(value) {
...
function validateUnique(uniqueValue, state) {
...
function validateNotNull(notNullValue, state) {
...
function parseDefaultValue(type, val) {
