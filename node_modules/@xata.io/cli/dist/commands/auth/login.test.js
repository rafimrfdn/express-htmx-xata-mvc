...
describe('auth login', () => {
    ...
    `);
        ...
        expect(fs.writeFile).toHaveBeenCalledWith(credentialsFilePath, ini.stringify({ default: { apiKey: '1234abcdef' } }), {
            mode: 0o600
        });
    });
    test("login with a provided API key and host provider and don't prompt for anything", async () => {
        ...
        expect(fetchMock.mock.calls[0][1].method).toEqual('GET');
        expect(fs.mkdir).toHaveBeenCalledWith(dirname(credentialsFilePath), { recursive: true });
        expect(fs.writeFile).toHaveBeenCalledWith(credentialsFilePath, ini.stringify({ default: { apiKey: 'foobar', api: 'production' } }), {
            mode: 0o600
        });
    });
});
