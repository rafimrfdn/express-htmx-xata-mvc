...
export function handler(publicKey, privateKey, passphrase, callback) {
    return (req, res) => {
        var _a, _b;
        try {
            if (req.method !== 'GET') {
...
export function generateURL(port, publicKey) {
    const pub = publicKey
        .replace(/\n/g, '')
...
export async function createAPIKeyThroughWebUI() {
    ...
    return new Promise((resolve) => {
        const server = http.createServer(handler(publicKey, privateKey, passphrase, (apiKey) => {
            resolve(apiKey);
            server.close();
        }));
        server.listen(() => {
