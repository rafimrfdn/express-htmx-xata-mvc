...
export function handler(webHost, publicKey, privateKey, passphrase, callback) {
    return (req, res) => {
        var _a, _b;
        try {
            if (req.method !== 'GET') {
            ...
            if (parsedURL.pathname === '/new') {
                const port = (_b = req.socket.localPort) !== null && _b !== void 0 ? _b : 80;
                res.writeHead(302, {
...
function renderSuccessPage(req, res, colorMode) {
    res.writeHead(200, {
...
export function generateURL(webHost, port, publicKey) {
    const pub = publicKey
        .replace(/\n/g, '')
...
export async function createAPIKeyThroughWebUI(webHost) {
    const { publicKey, privateKey, passphrase } = generateKeys();
    return new Promise((resolve) => {
        const server = http.createServer(handler(webHost, publicKey, privateKey, passphrase, (apiKey) => {
            resolve(apiKey);
            server.close();
        }));
        server.listen(() => {
            ...
            console.log(`We are opening your default browser. If your browser doesn't open automatically, please copy and paste the following URL into your browser: ${chalk.bold(`http://localhost:${port}/new`)}`);
