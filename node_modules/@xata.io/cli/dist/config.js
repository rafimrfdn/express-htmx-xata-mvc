...
export function setValue(path, schema, object, value) {
    const type = getType(path, schema);
    if (!type)
        throw new Error(`Unknown path "${path}"`);
    if (type === 'object')
        throw new Error(`Cannot set value at path "${path}" because it is an object`);
    set(object, path, parseValue(path, type, value));
}
function getType(path, schema) {
    let current = schema;
...
function parseValue(path, type, value) {
    if (type === 'boolean') {
        if (!['true', 'false'].includes(value))
            throw new Error(`Invalid boolean value "${value}" for path "${path}"`);
        return value === 'true';
    }
    if (type === 'number') {
        const num = +value;
        if (!Number.isFinite(num))
            throw new Error(`Invalid number value "${value}" for path "${path}"`);
        return num;
    }
    if (type === 'string') {
        return value;
